...
//  #logo teksta
        Assertions.assertEquals("Your Store", driver.findElement(By.xpath("//a[contains(@href, 'home')]")).getText());
...
//  antro produkto kaina (.product-layout .price) yra $123.20
        WebElement el = driver.findElement(By.cssSelector(".product-layout:nth-child(2) .price"));
        String text = el.getText().split("\n")[0].replace("$", "");
        Assertions.assertEquals("123.20", text);
//  patikrint kad Canon EOS 5D alt tekstas yra "Canon EOS 5D"
        Assertions.assertEquals("Canon EOS 5D", driver.findElement(By.cssSelector("div:nth-child(4).product-layout img")).getAttribute("alt"));
